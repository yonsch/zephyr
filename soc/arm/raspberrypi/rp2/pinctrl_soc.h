...
#ifndef ZEPHYR_SOC_ARM_RASPBERRYPI_RP2_PINCTRL_SOC_H_
#define ZEPHYR_SOC_ARM_RASPBERRYPI_RP2_PINCTRL_SOC_H_
...
#define Z_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)				\
	{									\
		...
		COND_CODE_1(DT_NODE_EXISTS(DT_PROP(node_id, drive_strength)),	\
			(DT_ENUM_IDX(node_id, drive_strength)),			\
			(DEFAULT_DRIVE_STRENGTH)),				\
		DT_PROP(node_id, slew_rate_fast),				\
...
#define DEFAULT_DRIVE_STRENGTH -1
#define RP2_GET_PIN_NUM(pinctrl) \
	(((pinctrl) >> PIN_NUM_POS) & PIN_NUM_MASK)
#define RP2_GET_PIN_ALT_FUNC(pinctrl) \
	(((pinctrl) >> ALT_FUNC_POS) & ALT_FUNC_MASK)

#endif /* ZEPHYR_SOC_ARM_RASPBERRYPI_RP2_PINCTRL_SOC_H_ */
